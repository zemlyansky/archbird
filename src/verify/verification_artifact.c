...
static int stable_identifier(const AbValue *value) {
  size_t index;
  if (!value || value->kind != AB_VALUE_STRING || !value->as.text.length)
    return 0;
  for (index = 0; index < value->as.text.length; index++) {
...
static int validate_tool(const AbValue *tool) {
  ...
  return object_fields_allowed(tool, allowed,
...
ArchbirdStatus ab_verification_finding_validate(ArchbirdEngine *engine,
  ...
  static const char *const comparisons[] = {
      "ambiguous", "different", "equal",        "extra",
      "missing",   "overlap",   "unregistered", "unresolved",
  };
  ...
  if (!object_fields_allowed(row, allowed,
                             sizeof(allowed) / sizeof(allowed[0])) ||
      !lowercase_sha256(ab_value_member(row, "fingerprint")) ||
...
static ArchbirdStatus validate_check(ArchbirdEngine *engine,
  ...
  if (!object_fields_allowed(row, allowed,
                             sizeof(allowed) / sizeof(allowed[0])) ||
      !stable_identifier(ab_value_member(row, "id")) ||
  ...
  for (index = 0; index < findings->as.array.count; index++) {
    ArchbirdStatus status = ab_verification_finding_validate(
        engine, &findings->as.array.items[index]);
    if (status != ARCHBIRD_OK)
      return status;
  }
  return ARCHBIRD_OK;
...
static int named_rows_unique(const AbValue *rows, const char *field,
  ...
  for (index = 0; index < rows->as.array.count; index++) {
    ...
    if ((identifiers && !stable_identifier(name)) ||
...
ArchbirdStatus ab_verification_artifact_load(ArchbirdEngine *engine,
  ...
  if (!object_fields_allowed(&out->root, root_allowed,
      ...
      !string_values_equal(ab_value_member(out->evaluation, "project"),
      ...
      diagnostics->kind != AB_VALUE_ARRAY || !lowercase_sha256(result_sha)) {
  ...
  for (index = 0; index < out->evaluations->as.array.count; index++) {
    ...
    if (!object_fields_allowed(evaluation, evaluation_allowed,
                               sizeof(evaluation_allowed) /
                                   sizeof(evaluation_allowed[0])) ||
        !stable_identifier(ab_value_member(evaluation, "id")) ||
        !lowercase_sha256(ab_value_member(evaluation, "map_config_sha256")) ||
        !lowercase_sha256(ab_value_member(evaluation, "map_input_sha256")) ||
        !lowercase_sha256(ab_value_member(
            evaluation, "map_producer_implementation_sha256")) ||
        !string_nonblank(ab_value_member(evaluation, "project")) ||
        !resolution ||
        (resolution->kind != AB_VALUE_NULL && !lowercase_sha256(resolution))) {
  ...
  for (index = 0; index < identities->as.array.count; index++) {
    ...
    if (!stable_identifier(ab_value_member(identity, "id")) ||
        !lowercase_sha256(
  ...
  status = value_digest(engine, &out->root, out->sha256);
  if (status != ARCHBIRD_OK)
    goto fail;
  return ARCHBIRD_OK;

fail:
  ab_verification_artifact_free(out);
  return status;
}
