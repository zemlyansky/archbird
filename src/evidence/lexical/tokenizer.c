...
static size_t utf8_scalar_length(const uint8_t *source, size_t length,
                                 size_t offset) {
...
ArchbirdStatus ab_utf8_validate(ArchbirdEngine *engine, const uint8_t *source,
  ...
  while (index < source_length) {
    size_t width = utf8_scalar_length(source, source_length, index);
    if (!width)
...
static size_t operator_end(const uint8_t *source, size_t length, size_t start) {
  ...
  {
    size_t width = utf8_scalar_length(source, length, start);
...
ArchbirdStatus ab_tokenize(ArchbirdEngine *engine, const uint8_t *source,
  ...
  while (cursor < source_length) {
    ...
    uint8_t value = source[cursor];
    if (value >= 0x80 &&
        utf8_scalar_length(source, source_length, cursor) == 0) {
