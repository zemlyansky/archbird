...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  (void)archbird_map_query_markdown_view(
      engine, fuzz_map_json, sizeof(fuzz_map_json) - 1, NULL, 0, data, size,
      ARCHBIRD_QUERY_VIEW_CHANGES, ARCHBIRD_REPORT_DETAIL_STANDARD, 0,
      fuzz_discard, NULL);
  (void)archbird_map_query_markdown_view_with_verification(
      engine, fuzz_map_json, sizeof(fuzz_map_json) - 1, NULL, 0, data, size,
      data, size, ARCHBIRD_QUERY_VIEW_CHANGES, ARCHBIRD_REPORT_DETAIL_STANDARD,
      0, fuzz_discard, NULL);
#else
  ...
  (void)archbird_map_render_markdown(engine, data, size, 1, 0, fuzz_discard,
                                     NULL);
  (void)archbird_map_query_markdown(
      engine, data, size, NULL, 0, fuzz_query_json, sizeof(fuzz_query_json) - 1,
      4096, fuzz_discard, NULL);
  (void)archbird_map_query_markdown_view(
      engine, data, size, NULL, 0, fuzz_query_json, sizeof(fuzz_query_json) - 1,
  ...
  (void)archbird_map_query_markdown_view_with_verification(
      engine, data, size, NULL, 0, fuzz_query_json, sizeof(fuzz_query_json) - 1,
      data, size, ARCHBIRD_QUERY_VIEW_CHANGES, ARCHBIRD_REPORT_DETAIL_COMPACT,
      4096, fuzz_discard, NULL);
