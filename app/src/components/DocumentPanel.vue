...
const preservedConstraints = computed(() =>
  Array.isArray(props.artifact.document.preserved_constraints)
    ...
    : 0);
const actTransitions = computed(() => rows(props.artifact.document.transitions));
...
<template>
  <section class="document-workspace">
    <header>
      ...
      <h1 v-else-if="artifact.artifact === 'act'">Architecture act</h1>
    ...
    <template v-else-if="artifact.artifact === 'act'">
      <div class="result-status" :data-status="artifact.document.state">
        <span>Act</span><strong>{{ artifact.document.state }}</strong>
      </div>
      <dl class="definition-list">
        <dt>Acceptance</dt><dd>{{ actAcceptance.status || 'not evaluated' }}</dd>
        <dt>Plan</dt><dd>{{ artifact.document.plan_sha256 }}</dd>
        <dt>File transitions</dt><dd>{{ actTransitions.length }}</dd>
      </dl>
      <article v-for="transition in actTransitions" :key="String(transition.path)" class="document-row">
